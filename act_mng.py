...
row1 = "Contact  : Frn13ds@gmail.com "
...
def show_accounts(username):
    if users[username]['accounts']:
        print(Back.YELLOW +"Account linked with tool account ")
        print(Fore.RED +"Seachring...")
        time.sleep(2)
        for account in users[username]['accounts']:
            print(Style.RESET_ALL +f"Account Name: {account['account_name']}, Password: {account['password']}")
    else:
